          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('adminId')}>
                  Admin Id <FontAwesomeIcon icon={getSortIconByFieldName('adminId')} />
                </th>
                <th>
            ...
            <tbody>
              {groupList.map((group, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {group.members
                      ? group.members.map((val, j) => (
                          <span key={j}>
                            <Link to={`/user-profile/${val.id}`}>{val.id}</Link>
                            {j === group.members.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        onClick={() => (window.location.href = `/group/${group.id}/delete`)}
                        color="danger"
                        size="sm"
