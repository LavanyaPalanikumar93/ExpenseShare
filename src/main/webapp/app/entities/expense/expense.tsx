            ...
            <tbody>
              {expenseList.map((expense, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{expense.user ? <Link to={`/user-profile/${expense.user.id}`}>{expense.user.id}</Link> : ''}</td>
                  <td>{expense.group ? <Link to={`/group/${expense.group.id}`}>{expense.group.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/expense/${expense.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`/expense/${expense.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button
                        onClick={() => (window.location.href = `/expense/${expense.id}/delete`)}
                        color="danger"
                        size="sm"
